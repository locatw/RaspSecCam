...
app_state_factory::app_state_factory(
	...
	: camera_(camera), capture_worker_(capture_worker), send_frame_worker_(send_frame_worker) 
{}
...
std::unique_ptr<app_state_factory::state_type> app_state_factory::create(app_state_id id)
{
	switch (id) {
		...
		case app_state_id::waiting_for_connection:
			return std::unique_ptr<state_type>(new waiting_for_connection_state(send_frame_worker_));
		case app_state_id::signaling:
			return std::unique_ptr<state_type>(new signaling_state(send_frame_worker_));
		case app_state_id::video_distribution:
			return std::unique_ptr<state_type>(new video_distribution_state(capture_worker_));
		default:
